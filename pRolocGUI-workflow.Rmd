...
The Bioconductor projet is well known for emphasising reproducible
research best practice through programmatic data analysis and we have
followed this principle in our efforts to provide robust and
rebroducible tools for the analysis of quantitative proteomics data
analysis. There are situtations where programmatic data manipulation
is however not the most efficient way to explore the data, and where
interactive graphical user interfaces (GUIs) offer great
benefits. Since the availability of the \CRANpkg{shiny} package
[@shiny], it has become relatively easy for R programmers to the
development of such interfaces. It however remains important to keep
the principles and reproducible and rigorous data analysis at the
finger tips of the users by allowing to easily transfer findings from
both interfaces, programmatic and graphical. 
...
defined in the `r Biocpkg("pRoloc")` package. `r Biocpkg("pRolocGUI")`
is intended for, but not limited to, the interactive visualisation and
analysis of quantitative spatial proteomics data. 
...
There are 3 applications distributed with `r Biocpkg("pRolocGUI")`
which are wrapped and launched by the `pRolocVis` function. These 3
applications are called according to the argument `app` in the
`pRolocVis` function which may be one of "main", "classify" or
"compare".
