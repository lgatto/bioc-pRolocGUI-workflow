...
This workflow describes the interactive GUIs implemented in the 
...
defined in the `r Biocpkg("pRoloc")` package. Building on the existing
functionality of these packages, `r Biocpkg("pRolocGUI")` works
straight out-of-the-box for already defined `MSnSet` datasets. This
gives users the benefit of interactive data exploration within a
formal and reproducible pipeline. This package is intended for the 
interactive visualisation and analysis of quantitative spatial 
proteomics data. 
...
`compare` and `aggregation`. Each app takes a `MSnSet` (or 
`MSnSetList`) data structure as input and features an interactive 
principal components analysis (PCA) plot of the data. 
Very briefly, the `main` application features a searchable 
and clickable PCA plot, data table and protein profiles plot. The 
`classify` app allows a user the visualisation of machine learning 
classification results. The `compare` app has been created to allow 
the comparsion of two `MSnSet` datasets. Finally, the `aggregation` 
app allows the comparison of the variability and effect of feature 
aggregation when aggregating spectra and/or peptides into protein 
groups. In the proceeding sections we describe the functionality and
the use-cases of each app in detail.
...
`hyperLOPIT2015` dataset from the `pRolocdata` package to showcase the interactive applications in `pRolocGUI`. This dataset is a high 
resolution spatial proteomics dataset generated from pluripotent mouse embryonic stem cells (E14TG2a) [@Christoforou:2016]. 
...
All the functions in the `pRolocGUI` 
package rely on an `MSnSet` object (or `MSnSetList` of length 2) as input. Users are referred to the 
`vignette("MSnbase-io")` or the `pRoloc` workflow for a step-by-step guide 
