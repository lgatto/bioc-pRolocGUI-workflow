...
benefits. Since the availability of the `r CRANpkg("shiny")` package
[xxx], it has become relatively easy for R programmers to
...
existing functionality of these packages, `r Biocpkg("pRolocGUI")`
works straight out-of-the-box for already defined `MSnSet` 
datasets. This allows users the benefit of interactive data 
exploration within a formal and reproducible pipeline. 
This package is intended for, but not limited to, the interactive 
visualisation and analysis of quantitative spatial proteomics data. 

We strongly recommend some familiarity with the `MSnSet` class 
(see `?MSnSet` for details) and the `pRoloc` vignette (see
